...
(() => {
  ...
  function createTaskEl(task) {
    ...
    li.classList.toggle('reorderable', !task.done && !isAndroid);
  ...
  function render() {
    ...
    if (isAndroid && state.tasks.length > 0 && !localStorage.getItem(SWIPE_SHOWN)) {
  ...
  function addTask(text, priority) {
    ...
    if (isAndroid && !localStorage.getItem(SWIPE_SHOWN)) {
      swipeHint.style.display = 'block';
      setTimeout(() => { swipeHint.style.display = 'none'; localStorage.setItem(SWIPE_SHOWN, '1'); }, 3000);
    }
  }
  ...
  function toggleDone(id, li, checked) {
    ...
    li.classList.toggle('reorderable', !checked && !isAndroid);
